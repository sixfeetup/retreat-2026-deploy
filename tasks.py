...
def remove_celery_files():
    file_names = [
        os.path.join("backend", "retreat_2026_deploy", "celery.py"),
        os.path.join("backend", "retreat_2026_deploy", "users", "tasks.py"),
        os.path.join(
            "backend",
            "retreat_2026_deploy",
            "users",
...
def configure_git_remote():
    repo_url = "git@github.com:sixfeetup/retreat-2026-deploy.git"
    if repo_url:
...
def party_popper():
    ...
    print("\r🎊 Congrats! Your retreat_2026_deploy project is ready! 🎉")
    print()
    print("To get started, run:")
    print("cd retreat_2026_deploy")
    print("tilt up")
    print()
...
def run_setup():
    subprocess.run(
        shlex.split("kind create cluster --name retreat-2026-deploy"), check=True
    )

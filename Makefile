...
ifeq ($(CI),true)
	...
	$(KUBECTL_EXEC_BACKEND) -c "cd retreat_2026_deploy && pytest --cov=./ --cov-report html --ds=config.settings.test"
endif
...
setup:
	...
	if [ "$$CURRENT_CONTEXT" = "kind-retreat-2026-deploy" ]; then \
		echo " $(GREEN)✔️$(END) The kubectl context is correctly set to kind-retreat-2026-deploy. Run 'tilt up' to start your cluster!"; \
	else \
		echo " $(YELLOW)⛭$(END) Current kubectl context is not 'kind-retreat-2026-deploy'. Switching context now..."; \
		if [ -z "$$(kubectl config get-contexts -o name | grep -w 'kind-retreat-2026-deploy$$')" ]; then \
			echo " $(YELLOW)⛭$(END) No context found for kind-retreat-2026-deploy. Creating one. Please wait, this may take a couple of minutes on a slower machine..."; \
			kind create cluster --name retreat-2026-deploy 1>/dev/null 2>/tmp/scaf_error.log; \
			echo " $(GREEN)✔️$(END) kind-retreat-2026-deploy cluster and context created."; \
			echo " $(BLUE)🗣️ Remember, you can safely run \"make setup\" any time to switch between Scaf projects.$(END)"; \
		fi; \
		kubectl config use-context kind-retreat-2026-deploy 1>/dev/null 2>/tmp/scaf_error.log; \
		echo " $(GREEN)✔️$(END) Context switched to kind-retreat-2026-deploy. Run 'tilt up' to start your cluster! "; \
	fi
...
secrets:
	...
	kubectl config get-contexts --no-headers | \
		grep -E "(admin@)?retreat-2026-deploy-$(ENVIRONMENT)" | \
		head -1 | \
		awk '{print $$2}' | \
	...
	envsubst < k8s/templates/secrets.yaml.template | op inject | if [ "$(DEBUG)" = "true" ]; then cat; else kubeseal -n retreat_2026_deploy-$(ENVIRONMENT) --format yaml > k8s/$(ENVIRONMENT)/secrets.yaml; fi

...
Configure kubernetes to your current project config and context, making sure you are in the correct prod/sandbox environment
    ...
    $ export KUBECONFIG=~/.kube/config:~/.kube/retreat_2026_deploy.ec2.config
    $ kubectl config use-context retreat_2026_deploy-environment
...
The decrypted values can be retrieved running:
    ...
    $ kubectl get secret secrets-config -n retreat-2026-deploy -o yaml > unsealed_secrets.yaml
